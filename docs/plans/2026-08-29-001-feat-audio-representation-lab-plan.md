...
Build SonicFusion's first learner-owned vertical slice: two custom DataFusion physical plans that render the same finite mono `sine -> gain` signal using different Arrow layouts. Each layout executes once, is collected once, and produces a WAV file plus a headless SVG waveform. The collected signals are canonicalized and compared before the learner records a representation decision.
...
- R4. Render a finite deterministic mono sine signal through gain offline.
...
- Milestone one is finite, offline, mono, and single-partition.
...
- DataFusion pull-based, bounded-memory execution overview: https://datafusion.apache.org/user-guide/arrow-introduction.html
...
- **Keep supporting dependencies explicit and narrow:** Use Tokio `=1.52.4` with only `macros` and `rt-multi-thread`, `futures = =0.3.31` for stream construction/utilities, `hound = =3.5.1` for float WAV, `thiserror = =2.0.20` for contextual project errors, and dev-only `tempfile = =3.20.0`. Commit `Cargo.lock`. Do not add direct Arrow or `async-trait` dependencies unless compilation against DataFusion `54.1.0` proves they are required. (`cargo check` confirmed DataFusion `54.1.0` requires Tokio `^1.52`.)
...
- **Make `frame_count` authoritative:** Samples occupy the half-open interval `0..frame_count`. Milestone one does not derive frames from a floating-point duration, avoiding an irrelevant rounding disagreement.
- **Bound the materialized lab by time:** A separate whole-second render limit defaults to 60 seconds and is caller-configurable. Validate `frame_count <= sample_rate × max_render_seconds` with checked arithmetic before plan construction; raising the limit is an explicit opt-in to larger in-memory vectors and artifacts.
...
- **Enforce one partition:** Sources advertise `UnknownPartitioning(1)`, incremental emission, and bounded execution through `PlanProperties`; they reject any partition index other than zero and preserve temporal ordering. `EquivalenceProperties` carries the schema and may declare the frame/start-frame ordering only after that claim has a monotonic-output test. Stream cursors belong to each `execute` call, never to the shared `ExecutionPlan`, so repeated executions are independent.
...
- **Use a thin builder, not a generalized audio framework:** The builder selects a layout and composes the corresponding layout-specific source and gain nodes. Shared interfaces stop at immutable configuration, physical-plan traits, and canonical sample output.
...
- **One collection per layout:** A layout's collected batches are converted once to an ordered sample vector, then both output writers consume that vector. The comparison executes the second layout independently; it does not introduce shared streaming fan-out.
...
- **Frame-count semantics:** Use explicit integer frames and half-open `0..frame_count` semantics.
...
flowchart TB
    ...
    Config --> Builder
    Builder --> FramePlan[Frame SineExec -> Frame GainExec]
    Builder --> BlockPlan[Block SineExec -> Block GainExec]

    FramePlan --> FrameStream[RecordBatch stream: frame rows]
    BlockPlan --> BlockStream[RecordBatch stream: block rows]
...
- [ ] U2. **Implement the frame-per-row sine source**

**Goal:** Learn a finite custom DataFusion leaf `ExecutionPlan` and `RecordBatchStream` using one row per audio frame.
...
- Implement an immutable one-partition, bounded leaf plan. `execute(0)` creates fresh stream-local cursor state; `execute(1+)` returns a clear error.
- Emit batches of at most `batch_frame_capacity`, calculate samples from absolute frame numbers, and emit a short final batch without dropping frames.
...
**Test scenarios:**
- Covers AE1. Happy path: 10 frames with batch capacity 4 emit three batches containing 4, 4, and 2 rows with frame values `0..10`.
...
- Independence: executing the same plan twice yields complete identical streams beginning at frame zero both times.
- Plan semantics: inspection reports the custom source, one output partition, and bounded execution; emitted frame indices are monotonic.

**Verification:**
- A multi-batch source produces every frame exactly once and no state lives in the reusable plan object.
...
- [ ] U5. **Implement the block-per-row sine source**

**Goal:** Rebuild the finite source with one nested processing block per Arrow row so the representation trade-off is experienced directly.
...
- Emit multiple block rows per RecordBatch, keep each normal row at `block_frames`, and represent only the final signal remainder as a shorter list.
...
- Plan semantics: source reports one bounded partition and monotonic `start_frame` output.
...
- Introduce a thin builder that accepts shared render/signal settings plus an explicit layout choice and returns the corresponding layout-specific `sine -> gain` physical tree.
...
- **Unchanged invariants:** Milestone one stays offline, finite, mono, and one-partition. No code path enters a real-time callback or claims multi-partition temporal semantics.
...
| A large render exhausts memory despite DataFusion's streaming model | Default the configurable render limit to 60 seconds. DataFusion's pull-based pipelines bound many intermediate working sets and spill-capable operators can use disk under a configured memory pool, but top-level `collect`, project-owned canonical vectors, and artifact writers do not automatically spill. |
...
- Frame and block signals contain exactly the configured number of finite, ordered frames and agree under the named absolute/relative policy across multi-batch, partial-block, and representative gain-scale cases.
